...
class calendar {
        ...
        switch ($cal) {
            ...
            case "largeMonth":
                ...
                if (((date("w", $date) == "0") || (date("w", $date) == "1")) && $this->showWeek) {
                    $week = " - <span style=\"font-size: 12px;\">第".date("W", $date)."周</span>";
                }
                if ($highlightDate) {
                ...
                $events = "<div style=\"font-size: 12px;font-weight: normal; color: ".$this->colorLargeFormatEventText."; width: 100%; height: ".$this->largeCellHeight."; overflow: auto;\">\n";
                $events .= "<div style=\"background-color: #E8EEF7;\"><span style=\"height: 16px;font-size: 10px;font-family: verdana;  color: ".$dateColor.";\">&nbsp;".date("j", $date).$week."</span><div>\n";
                break;
            case "weekly":
                //display the event with full text
            ...
            for ($i = 0; $i < sizeof($this->events); $i++) {
                ...
                if ($showEvent) {
                    ...
                    switch ($cal) {
                        ...
                        case "weekly":
                            //Display the event with full text.
                            $events .= "<div style=\"background-color: white;display: block;\"><span style=\"background-color: white;font-size: 10px;font-family: verdana;color:#ff6600;\">".date("h:i A", $this->events[$i]['date'])."</span>"."&nbsp;".$this->events[$i]['event']."</div>\n";
                            break;
                        case "weekly":
                    }
                }
    ...
    */
    function getMonth($m, $y) {
        ...
        switch (strtolower($this->monthFormat)) {
			...
			if ($month=='January'){$month='1月';}
			if ($month=='February'){$month='2月';}
			if ($month=='March'){$month='3月';}
			if ($month=='April'){$month='4月';}
			if ($month=='May'){$month='5月';}
			if ($month=='June'){$month='6月';}
			if ($month=='July'){$month='7月';}
			if ($month=='August'){$month='8月';}
			if ($month=='September'){$month='9月';}
			if ($month=='October'){$month='10月';}
			if ($month=='November'){$month='11月';}
			if ($month=='December'){$month='12月';}
                break;
            case "short":
    ...
    */
    function showSmallMonth($m, $y, $np = false, $showyear = true, $linkMonth = false) {
        ...
        //
        if ($np) {
            $prevLink = "<a href='".$_SERVER['PHP_SELF']."?mon=".$prevMonth."&yr=".$prevYear."&fmt=smallMonth".$get."' style=\"text-decoration: none;\"><img src='calendar/prev.gif' border=0></img></a> &nbsp;";
            $nextLink = " &nbsp;<a href='".$_SERVER['PHP_SELF']."?mon=".$nextMonth."&yr=".$nextYear."&fmt=smallMonth".$get."' style=\"text-decoration: none;\"><img src='calendar/next.gif' border=0></img></a>";
        } else {
            $prevLink = "";
            $nextLink = "";
        }
        ...
        //Create the header
        $output = "<div style=\"vertical-align: top;height:200px;background-color:#FFffff\">\n";
        $output .= "<table width=290px height=200px class=\"calendar\" border=\"".$this->smallMonthBorder."\"  cellspacing=\"0\" cellpadding=\"0\" align=\"center\"  style=\"font-size: 12px;border:1px solid #999999;border-collapse: collapse\">\n";
        $output .= "    <tr>\n";
        $output .= "        <th colspan=\"7\" style=\"height:33px;text-align: center;background-image:url(calendar/mctop.gif)\" >\n";
        $output .= "            <span style=\"font-size: 13px; font-weight: bold; color: #000000;\">".$prevLink.$month."&nbsp;".$year.$nextLink."</span>\n";
        $output .= "        </th>\n";
        $output .= "    </tr>\n";
        $output .= "    <tr style=\"height:28px;background-image:url(calendar/mctd.gif);color:'#000000';font-weight:bold\">\n";
        //now create the weekday headers
        if ($this->startingDOW == 0) {
            $output .= "        <td style=\"width: 14%; text-align: center;\">日</td>\n";
        }
        $output .= "        <td style=\"width: 14%; text-align: center;\">一</td>\n";
        $output .= "        <td style=\"width: 14%; text-align: center;\">二</td>\n";
        $output .= "        <td style=\"width: 14%; text-align: center;\">三</td>\n";
        $output .= "        <td style=\"width: 14%; text-align: center;\">四</td>\n";
        $output .= "        <td style=\"width: 14%; text-align: center;\">五</td>\n";
        $output .= "        <td style=\"width: 14%; text-align: center;\">六</td>\n";
        ...
        for($i=1; $i<=$days; $i++){
            if ($i == $day && $this->showToday) {
                $output .= $offset."        <td style=\"border:1px solid gray;width: 14%; text-align: center; color: ".$this->colorSmallFormatDateHighlight."; font-weight: bold;\">&nbsp;".$this->getEvents(mktime(0, 0, 0, $m, $i, $y), "smallMonth")."&nbsp;</td>\n";
            } else {
        ...
        $output .= "    </tr>\n";
		$output .="<tr><td colspan=7 style=\"height:23px;text-align: center;background-image:url(calendar/atd_bottom.gif)\"><div align=center><A HREF=\"calendar/large.php\">查看行事录</A></div></td></tr>";
        $output .= "</table>\n";
        $output .= "</div>\n";
    ...
    */
    function showLargeMonth($m, $y, $np) {
        ...
        $offset = "";
        if ($startDay > 0) {
            $offset .= "        <td width=\"14%\" bgcolor=\"#F0F3F8\" colspan=\"".$startDay."\">&nbsp;</td>\n";
        } else if ($startDay == -1) {
            $offset .= "        <td width=\"14%\" colspan=\"6\">&nbsp;</td>\n";
            $startDay = 6;
        }
        ...
        */
        ...
        $output .= "<table".$largeCalendarClass.$largeCalendarID." border=\"0\"  cellspacing=\"1\" cellpadding=\"0\" align=\"".$this->largeFormatAlign."\" style=\"background-color:#CCDDEE; width: ".$width.";".$height.$background."\">\n";
        $output .= "    <tr>\n";
        ...
        $output .= "            <span style=\"font-size: 30px; font-weight: bold; color: ".$this->colorLargeFormatHeaderText.";\">".$prevLink.$y."年".$month.$nextLink."</span>\n";
        $output .= "        </td>\n";
        ...
        //now create the weekday headers
        for ($i = 1; $i < 8; $i++) {
            $output .= "        <td style=\"background-color: #C3D9FF;width: 14%; text-align: center;\">".$this->getDOW($i)."</td>\n";
        }
        /*
        if ($this->startingDOW == 0) {
        ...
        */
        ...
        for($i=1; $i<=$days; $i++){
            ...
            if ((date("w",$date) == "0") || (date("w",$date) == "6") && isset($this->colorLargeFormatWeekendHighlight)) {
                $bgcolor = " background-color: ".$this->colorLargeFormatWeekendHighlight."; filter: alpha(opacity=100); -moz-opacity: 70%;";
            } else {
                $bgcolor = "";
            }
            if ($i == $day && $this->showToday) {
                ...
                $output .= $offset."        <td style=\"width: 14%;".$heightCalCell." vertical-align: top; text-align: left;background-color: white;\">".$this->getEvents($date, "largeMonth")."</td>\n";
            }
            $offset = "";
            $startDay ++;
            if ($startDay == 7) {
        ...
        if ($startDay > 0) {
            $output .= "        <td colspan=\"".(7 - $startDay)."\" style=\"background-color: #F0F3F8;width: 14%;".$heightCalCell."\">&nbsp;</td>\n";
        }
        $output .= "    </tr>\n";
        $output .= "</table>\n";
        $output .= "</div>";
