 ...
 * $Id: privilege.php,v 1.3 2008/08/11 16:07:00 root Exp $
*/

define('IN_EVP', true);

require(dirname(__FILE__) . '/includes/init.php');//知道了require的用法就是前面include,后名的页面再检查需求。不知道是不是，手册上没提到！
...
/* 初始化 $exc 对象 */
$exc = new exchange($evp->table("admin_user"), $db, 'user_id', 'user_name');
...
//    if (!empty($_SESSION['captcha_word']) && (intval($_CFG['captcha']) & CAPTCHA_ADMIN))
	  if (!empty($_SESSION['captcha_word']) && ( CAPTCHA_ADMIN))//CFG不知道什么定义的，先注释掉
    ...
    if ($row)
    {
        ...
        // 更新最后登录时间和IP
        $db->query("UPDATE " .$evp->table('admin_user').
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "SELECT nav_list FROM " . $evp->table('admin_user') . " WHERE action_list = 'all'";
    $row = $db->getRow($sql);

    $sql = "INSERT INTO ".$evp->table('admin_user')." (user_name, email, password, add_time, nav_list) ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT user_id, user_name, email, password, agency_id FROM " .$evp->table('admin_user').
    ...
    {
        $sql = "SELECT agency_name FROM " . $evp->table('agency') . " WHERE agency_id = '$user_info[agency_id]'";
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    {
        /* 查询旧密码并与输入的旧密码比较是否相同 */
        $sql = "SELECT password FROM ".$evp->table('admin_user')." WHERE user_id = '$admin_id'";
    ...
    //更新管理员信息
    $sql = "UPDATE " .$evp->table('admin_user'). " SET ".
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    /* 包含插件菜单语言项 */
    $sql = "SELECT code FROM ".$evp->table('plugins');
    $rs = $db->query($sql);
    ...
    $sql = "SELECT user_id, user_name, email, nav_list ".
           "FROM " .$evp->table('admin_user'). " WHERE user_id = '".$_SESSION['admin_id']."'";
    $user_info = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'allot')
{
    ...
    /* 获得该管理员的权限 */
    $priv_str = $db->getOne("SELECT action_list FROM " .$evp->table('admin_user'). " WHERE user_id = '$_GET[id]'");
    ...
    /* 获取权限的分组数据 */
    $sql_query = "SELECT action_id, parent_id, action_code FROM " .$evp->table('admin_action').
                 " WHERE parent_id = 0";
    ...
    /* 按权限组查询底级的权限名称 */
    $sql = "SELECT action_id, parent_id, action_code FROM " .$evp->table('admin_action').
...
elseif ($_REQUEST['act'] == 'update_allot')
{
    ...
    /* 取得当前管理员用户名 */
    $admin_name = $db->getOne("SELECT user_name FROM " .$evp->table('admin_user'). " WHERE user_id = '$_POST[id]'");

    /* 更新管理员的权限 */
    $act_list = @join(",", $_POST['action_code']);
    $sql = "UPDATE " .$evp->table('admin_user'). " SET action_list = '$act_list' ".
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 获得管理员用户名 */
    $admin_name = $db->getOne('SELECT user_name FROM '.$evp->table('admin_user')." WHERE user_id='$id'");
...
function get_admin_userlist()
{
    ...
    $sql  = 'SELECT user_id, user_name, email, add_time, last_login '.
            'FROM ' .$GLOBALS['evp']->table('admin_user').' ORDER BY user_id DESC';
...
function clear_cart()
{
    ...
    // 删除cart中无效的数据
    $sql = "DELETE FROM " . $GLOBALS['evp']->table('cart') .
