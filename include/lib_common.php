 ...
 * $Id: lib_common.php,v 1.4 2008/08/11 16:07:00 root Exp $
*/
...
{
    $arr = array();
//注意evp对象在init引入该脚本后到调用笨函数前被初始化了
    $sql = 'SELECT code, value FROM ' . $GLOBALS['evp']->table('shop_config') . ' WHERE parent_id > 0';
    $res = $GLOBALS['db']->getAllCached($sql);
...
function gzip_enabled()
{
    ...
    if ($enabled_gzip === NULL)
    {
	   ...
	   $enabled_gzip =  function_exists('ob_gzhandler');//强行做Gzip
    }
 ...
 * @return  void
 **/
function evp_header($string, $replace = true, $http_response_code = 0)
{
