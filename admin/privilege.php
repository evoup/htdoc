...
/**
 * ECSHOP 管理员信息以及权限管理程序evoupHACK版
 * ============================================================================
 ...
 * ============================================================================
 * $Author: root $
 * $Date: 2008/08/11 16:06:59 $
 * $Id: privilege.php,v 1.5 2008/08/11 16:06:59 root Exp $
*/
...
//这个实现，关联和替代是什么
//$exc = new exchange($ecs->table("users"), $db, 'id', 'real_name');
$exc = new exchange($evp->table("users"),$db, 'id', 'real_name');

//if (!is_a($exc,'exchange'))
...
{
    /* 清除cookie */
    //setcookie('EVP[admin_id]',   '', 1);
    //setcookie('EVP[admin_pass]', '', 1);
...
//echo $row['user_id'].'/'.$row['user_name'].'/'.$row['action_list'].'/'.$row['last_login'];
        ...
        if (isset($_POST['remember']))
        {
            ...
            //setcookie('EVP[admin_id]',   $row['user_id'],                            $time);
            //setcookie('EVP[admin_pass]', md5($row['password'] . $_CFG['hash_code']), $time);
        }
...
//if (isset($_SESSION['admin_id'])){die ('sessionadmin_id存在');}

//die("<hr>".$_SESSION['admin_id']."<hr>");
//die("LOGINOK");
/*echo "<script>window.location.href(\"./admin.php\")</script>";*/       
	   //die('ok');
	   echo ("<script>location.href='./admin.php'</script>");
	   //die;
	    //evp_header("Location: ./admin.php");//转入后台页//这里还要测试header要比js的location.href快得多
...
function clear_cart()
{
    ...
    $sql = "SELECT DISTINCT session_id " .
            "FROM " . $GLOBALS['evp']->table('cart') . " AS c, " .
                $GLOBALS['evp']->table('sessions') . " AS s " .
